...
def fetch_data(symbols, period_days, interval="1d"):
    ...
    start_date = end_date - timedelta(days=period_days + 50)  # Add buffer for calculations
...
def calculate_jdk_rs_momentum(rs_ratio, period=5):
    ...
    OR simplified as: RS-Momentum = Rate of Change of RS-Ratio over period
    
    This measures the momentum/speed of the RS-Ratio trend
    """
    if rs_ratio is None or len(rs_ratio) < period:
        return None

    # Method 1: Rate of change approach (more common)
    # Shows percentage change in RS-Ratio over the period
    momentum = ((rs_ratio / rs_ratio.shift(period)) - 1) * 100
    
    # Method 2: Alternative - normalized momentum (uncomment to use)
    # rs_ratio_ma = rs_ratio.rolling(window=period).mean()
    # momentum = ((rs_ratio / rs_ratio_ma) - 1) * 100
...
def create_animated_rrg_plot(results, animation_period_days, animation_speed=800):
    """Create animated Relative Rotation Graph"""
    
    # Collect all data points for each symbol over time
    all_frames_data = []
    dates = []
    ...
    # Filter dates to only include the selected period
    end_date = dates[-1]
    ...
    if animation_period_days <= 7:  # 1 day to 1 week - daily
        ...
        step = 5
    elif animation_period_days == 365:  # 1 year - bi-weekly
        step = 10
    elif animation_period_days == 730:  # 2 years - bi-weekly
        step = 15
    else:  # 3 years - monthly
        step = 20
    ...
    # Calculate global ranges for consistent axis
    all_rs_ratios = []
    all_rs_momentum = []
    ...
    # Calculate y_range and ensure 0 is visible
    ...
    for frame_idx, date in enumerate(sampled_dates):
        ...
        for i, (symbol, data) in enumerate(results.items()):
            ...
            try:
                ...
                # Get trail (last 10 points)
                trail_length = min(10, len(rs_ratio))
        ...
        frames.append(go.Frame(
            ...
            name=str(frame_idx),
            layout=go.Layout(
                title_text=f"Sector Rotation - {date.strftime('%Y-%m-%d')}"
            )
        ))
    ...
    fig.update_layout(
        ...
        width=800,
        height=1000,
        title=f"Sector Rotation - {sampled_dates[0].strftime('%Y-%m-%d')}" if sampled_dates else "Sector Rotation",
        showlegend=True,
        legend=dict(
            orientation="v",
            yanchor="top",
            ...
            'steps': [
                {
                    ...
                    'label': sampled_dates[int(f.name)].strftime('%Y-%m-%d'),
                    'method': 'animate'
                }
                for f in frames
            ]
...
def create_static_rrg_plot(results, tail_length, show_tail=False, enable_smoothing=False):
...
def main():
    ...
    with col1:
        ...
        tail_length = st.slider("Count Days (Tails length)", min_value=2, max_value=30, value=10, step=1,
    ...
    if st.button("Run Analysis", type="primary"):
        ...
        with st.spinner("Fetching data and calculating metrics..."):
            ...
            if enable_animation:
                ...
                fig = create_animated_rrg_plot(results, period, animation_speed)
            else:
